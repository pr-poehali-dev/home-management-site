...
const News = () => {
  ...
  const fetchNews = async () => {
    try {
      ...
      const hasTargetFeeNews = data.news?.some((n: NewsItem) => n.title.includes("ЕДИНОВРЕМЕННЫХ ЦЕЛЕВЫХ ВЗНОСОВ"));

      const targetFeeNews: NewsItem = {
        id: 1003,
        title: "УТВЕРЖДЁН РАЗМЕР ЕДИНОВРЕМЕННЫХ ЦЕЛЕВЫХ ВЗНОСОВ",
        date: "2026-06-17",
        tag: "ВНИМАНИЕ",
        content: `УВАЖАЕМЫЕ СОБСТВЕННИКИ!\n\nНапоминаем, что в соответствии с Протоколом № 20251100102593 от 21.11.2025 г. внеочередного общего собрания собственников (правообладателей) помещений в многоквартирном доме, расположенном по адресу: Ленинградская область, Всеволожский муниципальный район, Бугровское городское поселение, город Бугры, Петровский бульвар, дом 28, проведенного в форме очно-заочного голосования в период с 11.11.2025 по 17.11.2025\n\nУТВЕРЖДЕН РАЗМЕР ЕДИНОВРЕМЕННЫХ ЦЕЛЕВЫХ ВЗНОСОВ\nна оборудование общего имущества и вывоз строительных крупногабаритных отходов\n(срок оплаты – не позднее 6 месяцев с момента утверждения на общем собрании собственников):\n\n1. Приобретение дополнительного имущества/оборудования, включаемого в состав общего имущества МКД, в целях содержания и обслуживания ОДИ, услуг и/или работ ООО «УК «Наш дом - Регион» в размере 59 руб. 14 коп./кв.м жилых/нежилых помещений (без учета НДС).\n\n2. Вывоз строительных крупногабаритных отходов, в случае если помещение передается Застройщиком без отделочных работ, в размере 133,20 рублей/кв.м жилых/нежилых помещений (без учета НДС).\n\n3. Вывоз строительных крупногабаритных отходов, в случае если помещение передается Застройщиком с проведением отделочных работ, в размере 88,80 рублей/кв.м жилых/нежилых помещений (без учета НДС).`
      };

      const passportOfficeNews: NewsItem = {
        id: 1004,
        title: "Паспортный стол временно не работает",
        date: "2026-06-19",
        tag: "ВНИМАНИЕ",
        content: `ООО "УК "НОВОЕ КУПЧИНО"\n\nУважаемые жители!\n\nВ связи со смертью сотрудника паспортного стола, паспортный стол временно не работает. Информацию о возобновлении работы опубликуем отдельно.`
      };
      ...
      newsToAdd.push(passportOfficeNews);
      if (!hasScamWarning) newsToAdd.push(scamWarningNews);
      if (!hasTargetFeeNews) newsToAdd.push(targetFeeNews);
